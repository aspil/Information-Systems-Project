...
static void heapify_up(struct heapq *heap, int id) {
    ...
    if (heap->compare(vector_get(heap->vector, parent_node-1), vector_get(heap->vector, id-1)) < 0) {
...
static void heapify_down(struct heapq *heap, int id) {
    ...
    if (right_child <= size && heap->compare(vector_get(heap->vector, left_child-1), vector_get(heap->vector, right_child-1)) < 0)
        max = right_child;
