...
struct hash_map *map_init(unsigned int		  size,
						  HashFunction		  hash,
...
void *map_advance(struct hash_map *map)
{
	...
	if (pos != map->size - 1) {
		temp = map->iterator;
		// map->iterator = (map->iterator != map->last_chain_bucket[pos]) ? temp->next :
		// map->array[pos+1];
...
// 			}
// 			printf("\tclique's first product: %s - %d\n", (clique->first_product)->website,
// 				   (clique->first_product)->id);
// 			printf("\tclique's last product: %s - %d\n", (clique->last_product)->website,
// 				   (clique->last_product)->id);
// 			temp = temp->next;
// 		}
// 		printf("\n");
// 	}
